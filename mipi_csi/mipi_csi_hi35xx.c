...
static int CheckLaneId(struct MipiCsiCntlr *cntlr, uint8_t devno, InputMode inputMode, const short pLaneId[])
{
    ...
    OsalSpinUnlock(&cntlr->ctxLock);
    if (CheckLane(devno, laneNum, curLaneDivideMode, &laneSum, pLaneId) == HDF_FAILURE) {
...
static int MipiSetComboDevAttr(struct MipiCsiCntlr *cntlr, const ComboDevAttr *pAttr)
{
    int ret;
...
static long SetComboDevAttr(struct MipiCsiCntlr *cntlr, const ComboDevAttr *argp)
{
    int32_t ret;
    errno_t err;
...
static int32_t Hi35xxMipiCsiInit(struct HdfDeviceObject *device)
{
    ...
    HDF_LOGI("%s: load mipi csi driver 1212 success!", __func__);
...
static void Hi35xxMipiCsiRelease(struct HdfDeviceObject *device)
{
    ...
    HDF_LOGI("%s: unload mipi csi driver 2121 success!", __func__);
}
